...
namespace eProject.Service
{
    public class AuctionServices : IAuctionServices
    {
        ...
        private List<CatAuctionUser> products = new List<CatAuctionUser>();

        public IEnumerable<CatAuctionUser> getProductAll()
        {
            return products;
        }
        ...
        public IEnumerable<Auction> paginationProduct(int start, int limit)
        {
            var data = context.Auctions.ToList();
            var dataProduct = data.OrderByDescending(x => x.AuctionId).Skip(start).Take(limit);
        ...
        public bool LockAuction(int id)
        {
            var auction = context.Auctions.SingleOrDefault(a => a.AuctionId.Equals(id));
            if (auction != null)
            {
