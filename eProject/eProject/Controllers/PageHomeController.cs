...
namespace eProject.Controllers
{
    public class PageHomeController : Controller
    {
        ...
        private readonly INews serviceNews;

        private readonly IWinner serviceWin;
        private readonly Data.DatabaseContext db;
        public PageHomeController(IAuctionServices serviceAuction, IAuctionBid serviceBid, IUserServices serviceUser, Data.DatabaseContext db, IWinner serviceWin, INews serviceNews)
        {
        ...
        public void UpadateAuction()
        {
            //The auction's winner update timed out
            var model = db.Auctions.ToList();
            foreach (var item in model)
            {
                if (item.EndDate < DateTime.Now)
                {
                    //Update status of Auction from Active to Inactive
                    serviceAuction.InactiveAuction(item.AuctionId);
                    //Update Winner
                    var bid = db.AuctionBids.ToList().Where(x => x.AuctionId.Equals(item.AuctionId));
                    if (bid != null)
                    {
                        ...
                        if (bidmax != null)
                        {
                            serviceWin.UpdateWinner(item.AuctionId, bidmax.UserId);
                        }
                    }
                }                
            }
        ...
        {
            UpadateAuction();
            var model = serviceAuction.ListAuctions().Where(x=>x.Auction.Status=="Active");           
            ViewBag.End = model.Where(x => (x.Auction.EndDate - DateTime.Now).TotalDays < 1).ToList().OrderBy(x=> (x.Auction.EndDate - DateTime.Now).TotalHours);           
            ViewBag.Hot = model.OrderByDescending(x => serviceBid.GetBids(x.Auction.AuctionId).Count());         
            ViewBag.Review = db.Ratings.ToList();
            ViewBag.News = serviceNews.GetNews();
        ...
        public IActionResult SendMail(string Name, string Email, string Message)
        {
            ...
            email.To.Add(MailboxAddress.Parse("thinhphanlhc@gmail.com"));

            string MailText = "<!DOCTYPE html><html lang='en' xmlns='http://www.w3.org/1999/xhtml' " +
                ...
                "<tr> <td valign='middle' class='bg_light footer email-section'> <table> <tr> <td valign='top' width='33.333%' style='padding-top: 20px;'> <table role='presentation' cellspacing='0' cellpadding='0' border='0' width='100%'> <tr> <td style='text-align: left; padding-right: 10px;'> <h3 class='heading'>About</h3> <p>Online Auction for making your market actions in order to reach a much larger audience.</p></td></tr></table> </td><td valign='top' width='33.333%' style='padding-top: 20px;'> <table role='presentation' cellspacing='0' cellpadding='0' border='0' width='100%'> <tr> <td style='text-align: left; padding-left: 5px; padding-right: 5px;'> <h3 class='heading'>Contact Info</h3> <ul> <li><span class='text'>590 Cach Mang Thang Tam, Ward 11, District 3, Ho Chi Minh City, Vietnam </span></li><li><span class='text'>+84 909 123 123</span></a></li></ul> </td></tr></table> </td><td valign='top' width='33.333%' style='padding-top: 20px;'> <table role='presentation' cellspacing='0' cellpadding='0' border='0' width='100%'> <tr> <td style='text-align: left; padding-left: 10px;'> </td></tr></table> </td></tr></table> </td></tr></table> </div></center></body></html>";


            email.Subject = "Guest Question";
